...
"use strict";
  ...
  function addMove(id) {
    ...
    if ( ! playerSymbol ) {
      return alert("Please choose a symbol to start playing.");
    }
    
    // Create an ID from the click event.
    var field = "#"+id;
    
    // Make sure the field is empty, prevent accidentally clicking the same field twice.
    if ( $(field).html().length > 0)  {
      return alert("This field has already been played. Please select a different field.");
    }
